...
-- Momentum
CREATE OR REPLACE FUNCTION tap_mom(_interval INT, _instrument_id BIGINT, _from DATE, _to DATE)
  RETURNS TABLE("close_date" DATE, "mom" DOUBLE PRECISION)
LANGUAGE plpgsql
AS $$
DECLARE
BEGIN
  ...
  FROM (
       SELECT
              today.close_date,
              today.close_price - yesterday.close_price AS mom
       FROM prices today
              INNER JOIN
                LATERAL (
