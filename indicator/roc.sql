...
-- Rate of Change
CREATE OR REPLACE FUNCTION tap_roc(_interval INT, _instrument_id BIGINT, _from DATE, _to DATE)
  RETURNS TABLE("close_date" DATE, "roc" DOUBLE PRECISION)
LANGUAGE plpgsql
AS $$
DECLARE
BEGIN
  ...
  FROM (
       SELECT
              today.close_date,
              (today.close_price - yesterday.close_price) / yesterday.close_price AS roc
       FROM prices today
              INNER JOIN
                LATERAL (
